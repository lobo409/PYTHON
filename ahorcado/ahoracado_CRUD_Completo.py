...
if opcion == "1":
    ...
    while intentos_login > 0:
        ...
        cursor.execute("SELECT ID, Nombre, Contraseña FROM usuarios WHERE Nombre = %s", (usuario,))
        resultado = cursor.fetchone()
        nombre_bd = resultado[1]
        hash_contraseña_bd = resultado[2]
        hash_contraseña_ingresada = hashlib.sha256(contraseña.encode()).hexdigest()
        if usuario == nombre_bd and hash_contraseña_ingresada == hash_contraseña_bd:
    ...
    opcion2 = input("Indique una opcion: ")
    if opcion2 == "1":
        nuevonombre = input("Ingresa tu nuevo nickname para el usuario: ")
        actualizar_nombre = "UPDATE usuarios SET Nombre = %s WHERE ID = %s"
        cursor.execute(actualizar_nombre, (nuevonombre, id_usuario))
        print("¡Nombre de usuario actualizado correctamente!")
    elif opcion2 == "2":
        nuevapasswd = getpass.getpass("Ingresa una nueva contraseña para el usuario: ")
        hash_passwd = hashlib.sha256(nuevapasswd.encode()).hexdigest()
        actualizar_passwd = "UPDATE usuarios SET Contraseña = %s WHERE ID = %s"
        cursor.execute(actualizar_passwd, (hash_passwd, id_usuario))
